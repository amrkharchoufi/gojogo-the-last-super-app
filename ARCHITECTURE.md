...
**Live packages today:** `com.gojogo.auth`, `profile`, `social`, `media`, `music`, `messaging`, `notifications`, `economy`, `delivery` (plus app-level `SecurityConfig` / `ApiExceptionHandler`). `notifications` is the first cross-module event consumer (listens to `social`'s `UserFollowed` / `PostLiked` / `PostCommented` / `StoryReacted` / `StoryReplied`, and since M5 also to `messaging`'s `MessageSent` for chat-message push). `economy` is the first Phase 2b vertical — publishes `ListingCreated` (no consumer yet; the search index is a later 2b slice).
