...
- **A token pack is bought out of the wallet, not off a card** (Phase 3 M4, 2026-08-01). The capture table below says "Stripe → `TOKENS` credit at pack rate"; the first bullet above already settled that card money only ever arrives as a top-up, and a second Stripe path for packs would mean a second webhook, a second failure mode and a second refund story. So a pack is `AVAILABLE → TOKENS` and a short wallet is a 402 with the shortfall — the same screen the driver stake uses. A pack's **bonus is a second entry funded by `PLATFORM`**: a discount credited with no debit is a ledger that stops adding up.
- **A statement shows every movement between your own pockets except a hold** (Phase 3 M4). The original rule dropped all of them as bookkeeping, which was right for a hold and wrong for everything else: buying tokens and locking a stake both take money out of what you can spend, and dropping them left a balance that fell with no line to explain it. Own-bucket lines are signed **from the AVAILABLE side**. A pocket also has its own history signed from *itself* (`bucketStatement`), because the same purchase is money out on a wallet and eleven tokens in on a token screen.
...
- **A worker withdrawal is capped at what they earned, and the surface lives in `dispatch`** (Phase 4 M2, 2026-08-02). This spec's payee list has always included couriers and drivers; building it exposed something the merchant case hides. A merchant is paid into a `MERCHANT` account, which has no way to receive a top-up — so its balance is *by construction* money earned. A driver or courier is paid into their own **`USER` `AVAILABLE`** bucket, the same pocket a card top-up lands in, so a payout surface over a raw balance is the first **card→bank path** this platform has ever had. The cap is `min(available, max(0, lifetime earned − lifetime paid out))`, over the earning kinds `COURIER_FEE | TIP | RIDE_FARE | REWARD`; over the ceiling is a **409 naming the number**, never a silent clamp. Two rejected alternatives worth recording: a separate `EARNINGS` bucket (correct, and it would mean every vertical learning to pay into a second pocket and a migration for money already paid), and a Stripe-side limit (invisible to the person, and enforced by somebody else's dashboard). **The hole it does not close, stated rather than discovered:** earnings are counted *gross*, so somebody who spends their fares and then tops up can still move card money out, bounded by a lifetime of work. What holds either way is the invariant that matters — nobody withdraws more than the platform has genuinely paid them.
- **The aggregate this needed had to be split in two, and the split is not pedantry.** `WalletApi.totalByKind` counts the **credit side only** — an entry counts when one of this owner's accounts is *receiving* — because a vehicle-verification `REWARD` moves out of one driver's `STAKING` into another person's `REWARDS`, and a two-sided sum would credit the driver who *funded* a reward with the earnings of the passenger who collected it. Lifetime paid-out is therefore **not** a sum over the `PAYOUT` ledger kind but over `payments.payout` rows (`REQUESTED` + `SENT`), because a payout Stripe refuses leaves its debit standing and reverses it with an `ADJUSTMENT` — the ledger is append-only — so summing the kind would hold a transfer that never happened against a worker forever. Each half lives with the module that owns the fact.
...
## 4. Driver/courier onboarding logic (`partner` + wallet + dispatch) — **BUILT 2026-08-01**, community verification **2026-08-02**
...
- ~~**Ride-hailing tokens are M4**, not built here.~~ **BUILT 2026-08-01 (Phase 3 M4).** Refinements below.

**Community vehicle verification — refinements the M5 build made (2026-08-02):**

- **The vehicle id had to be made to travel, and it travels the way the plate already did.** A verification has to land on the car the passenger actually rode in, not on whatever the driver has made active by the time they answer — so `partner` pushes a **`VehicleRef`** (id, category, label, plate, verified) down to the dispatch registration at provisioning and on every vehicle change, dispatch hands it back on an `Assignment`, and `travel` snapshots it onto the ride at CONFIRMED. `travel → partner` still does not exist. The badge rides in the same record for the same reason: a badge nobody sees is not a badge.
- **The invitation and the answer are one row, and it is unique on the ride.** The unanswered state is the interesting one — it is what the sweep looks for and what "we already asked somebody" means — and uniqueness on `ride_id` is what makes a redelivered `TripCompleted` harmless. A **sweep** marks lapsed invitations `EXPIRED`; without it the "is one still out?" check finds a row from three weeks ago forever and the fall-through never happens.
- **Silence is not an accusation.** An expired invitation flags nothing. Treating it as a complaint would take drivers off the road for having busy passengers.
- **All five, or it is a rejection — and a *missing* answer counts as no.** Each question is a different way the car could be wrong, so a badge surviving one "no" says nothing; and an older client omitting a field must not be able to verify a car by omission.
- **The same passenger is never asked twice about the same car**, which is how one friend's five trips would otherwise become a badge. Neither is the driver.
- **The reward is capped at what is left of the stake and recorded on the application** (`partner_account.reward_paid_minor`, folded into `remainingStakeMinor` beside the KYC fee). Without the column a driver whose stake has paid three rewards still looks fully staked, and a release hands back money that is already somebody else's. A driver with nothing left pays nothing and the badge still stands — it is about the car, not the balance.
- **A rejection flags the vehicle *and* suspends its driver in one transaction**, the same rule the papers-expiry sweep follows: a flag that leaves work arriving is a flag that did nothing. Every passenger verdict is attached to the vehicle on the review surface, so a moderator can read why.
- **Not built:** the optional passenger **photos** (the five answers were the blocking half; the private-prefix upload path exists and can be added without a migration), and any re-invitation after the cap.

**Ride-hailing tokens — refinements the M4 build made (2026-08-01):**

- **The price list is split across two modules, and that is the milestone's main boundary call.** `payments.token_pack` (a size and a price) is here as specified. The *ride* price — spec'd as `payments.token_policy` — moved to **`travel.ride_token_price`**, because it is keyed on a `dispatch.VehicleCategory` and a distance in metres, which is vocabulary a platform money module has no business learning and could only store as strings it cannot check. Next to `travel.pricing_config` it is what it actually is: the other half of the same price change. Payments sells tokens; travel says what they buy.
- **A token is a fixed number of minor units, and the TOKENS bucket holds money like every other bucket.** The ledger has one amount per entry, so a bucket counting tokens would make a purchase an entry whose two sides are in different units. `payments.token.value.minor` (default 100) is the only conversion in the system, and it lives behind a new public **`TokenApi`** so no vertical has a second copy of what a token is worth.
- **Bands are upper bounds, not ranges.** The winning row is the cheapest `up_to_metres` that covers the trip, inclusive at the top, and each category ends with a catch-all. A (min, max) pair can be left with a gap, and a gap is a ride that silently costs nothing. Nothing covering the distance falls to **zero**, not to the dearest band: free trips get reported, overcharged ones get left over.
- **Dispatch filters, and still knows nothing about money.** SPECS says a driver below the minimum "is filtered out by dispatch, not blocked from the app" — so dispatch gained **`WorkerEligibility`**, an SPI it defines and `travel` implements, the third instance of the `ConversationGuard` / `ModeratableContent` shape. It is asked **per job**, because a short hop and a cross-city trip cost different numbers of tokens. With no implementations on the classpath everybody is eligible. The check runs **after ranking and before the wave limit**, so an ineligible driver is reached past rather than shrinking the wave.
- **Being filtered out is silent, so the explanation lives on `GET /v1/travel/tokens`.** Dispatch simply stops offering — correct, and invisible. Without a sentence from the module that prices a ride, a driver sits online watching nothing arrive and concludes the app is broken.
- **The charge never blocks a confirmation.** Prevention is the eligibility filter; reaching CONFIRMED without the tokens needs the balance to move between the offer and the handshake, which the busy flag already prevents. Refusing there would take a car away from a rider who has been promised one, over the driver's finances.
- **Tokens follow the trip, not the fault.** The spec refunds only on a driver-fault cancellation; this refunds on **any** cancellation before completion. Keeping them would make cancelling a revenue line for the platform and would charge a driver for a rider's change of mind, which teaches drivers to decline. A driver who accepts and cancels repeatedly is already priced — dispatch counts every one against them.
- **The low-balance push fires on the spend**, not from a sweep: a balance sitting at one token for a week is not news, and a nightly job announcing it to everybody who has ever driven is the notification people turn off. Zero and "nearly out" are worded apart, because at zero the driver has already stopped being offered work.
- **New, and not in the spec: a welcome grant at approval** (`payments.tokens.welcome.grant`, default 5). Without it this milestone ships a silent stop — an approval hands somebody a registration, an empty token balance, and a Driver Mode where nothing ever arrives. Drivers only; a courier does not spend ride tokens.
- **Not built:** effective-dated *packs* (packs are `active` + `sort_order`, since a price change is an edit rather than a schedule), token expiry, and any refund of tokens to money.
...
- **The code direction is reversed: the merchant displays it and the courier types it.** This section says the courier shows a code and the merchant confirms it. Built the other way round, because the party who must act should be the party with the incentive. A merchant who is busy and forgets to tap leaves a courier standing at a counter with food they cannot mark collected and no way to proceed; a courier cannot be paid until they complete the step, and the only way for them to learn the code is to be standing at that counter — which is the exact fact the code exists to prove. The QR is the same code rendered on the kitchen's screen, not the courier's.
- **A failed handoff leaves the order `DELIVERING`, because there is no `ARRIVED`.** This section says "order stays `ARRIVED`"; that status has never existed and M2 did not add it, for the reason M1 refused to add one. A courier who cannot complete a handoff is still carrying the food, which is what `DELIVERING` means.
- **A wrong code is an answer, not an error.** Both handoff verbs return **200** with `HandoffResultDto(accepted, message, attemptsLeft, supportUnlocked, job)`. A courier who typed a 7 for a 1 has not made a bad request, and an error banner is the wrong thing to put in front of somebody holding a bag of food. It also means the attempt counter increments inside a committed transaction rather than being rolled back by a thrown exception — which is the mechanical reason this shape is not merely a nicety.
- **Attempts are counted at the door and deliberately not at the counter.** The asymmetry is the point. At the counter the merchant is standing right there, so the code is a *check*: a courier who fails it must not be locked out of food they are already holding, so pickup never counts and never locks (`attemptsLeft = -1`, meaning "not counted"). At the door the risk runs the other way — a courier marking an undelivered order delivered — so wrong PINs are counted, and hitting the maximum opens the photo fallback rather than stranding anybody.
- **An empty PIN counts as a wrong attempt.** Not an oversight: a courier at a door nobody answers has no PIN to type, and if "no PIN" cost nothing they could never reach the fallback built for precisely their situation. Three taps is the honest way out of a silent doorstep.
- **The fallback after lockout is a deliberate, documented hole.** Once attempts reach **CONFIG** 3, a PIN order may be completed by the photo path. It can be abused — fail three times on purpose, photograph a doorstep — and the alternative is worse: a courier stranded in the street with food nobody will accept because the customer is asleep or gave the phone to a child. Mitigations: the attempts are recorded on the row and never reset, the photo is evidence in a dispute, and the customer sees both. A mechanism whose failure mode is "the delivery does not happen" is not a safety mechanism.
- **No endpoint anywhere accepts a client-supplied object key, and the server picks the format.** The proof-photo presign stamps the key it minted onto the order in the same transaction, so there is no attach step and therefore no prefix check to get wrong. One content type (`image/jpeg`) means the presigned PUT and the client's header cannot disagree — the mismatch that looks like a broken upload and is actually a bad signature. Replacing a photo deletes the object behind the old key, since nothing sweeps the private prefix.
- **Handoff mode is a column on the order, not a status and not an account setting.** "Leave it at the door" is what somebody types when the courier is four minutes away and the baby has just gone down, so it is set at checkout, changeable in flight until delivered, and read at the moment of the handoff rather than snapshotted at assignment. An unrecognised word falls back to the configured default instead of failing a checkout.
- **Tips did not need building.** §5's handoff paragraph groups "tips through the wallet" with the codes; 2e M3 built the tip at checkout and 4 M1 redirected it to the courier, so M2 changed nothing here. What M2 added is the *withdrawal* — see §1 below.

Refinements the Phase 4 M1 build made to §3's courier trigger and to this section:

- **The kitchen had to become a person before the courier could.** Deleting `OrderFulfilmentJob` removes the only thing that ever moved an order, so a merchant order queue is not scope creep — it is what "delete the simulation" costs. CONFIRMED now means *waiting for the restaurant*; they accept with a prep estimate, and that estimate is the first honest `readyAt` this vertical has ever had. `/v1/delivery/merchants/mine/orders` + accept / reject / ready, on the owner's existing `/mine` surface.
- **`readyAt − pickupLead` is `DispatchRequest.startAfter`, and nothing else.** §3 predicted this ("the wave clock doubles as the courier trigger") and M1 built the field; this is the first caller. A courier search that must not start until the kitchen is nearly done is a job whose first wave is in the future — no new mechanism, no poller of delivery's own.
- **No new order status, deliberately.** The six are unchanged, which is the whole point of the swap 2b M4 designed. "The food is ready and nobody has taken the delivery" is not a stage of an order — it is a problem with one — so it is a **`courier_search`** column (NONE / SEARCHING / ASSIGNED / FAILED) beside the status, not a seventh word every client would have to learn. Same for "cooked": a timestamp, because the customer's food is still coming and their screen still says so.
- **An exhausted search is not a cancellation, and there is no second search.** Dispatch's job row is unique on (kind, ref) by design, so re-asking would need a platform verb to re-open a closed job — and it is the same question to the same empty city, since every ring within range was searched for the whole request window. The order is flagged, both sides see it, and the customer may cancel for a full release. Auto-cancelling would throw away food that may already be made, which is a decision for the people involved rather than for a sweep.
- **A restaurant that never answers is the failure the simulation could not have.** `OrderTimeoutJob` cancels and *releases* after **CONFIG** 10 minutes. The same query collects the orders stranded by this milestone's own deploy — an order in flight when the fulfilment job was deleted also has no `acceptedAt` — which is why the migration deliberately cancels nothing itself: releasing an escrow hold is a ledger movement, and a row marked CANCELLED with the money still held is the one unacceptable outcome.
- **The courier's REST surface is two verbs.** Going online, position, offers and accept are *dispatch's* and identical for a driver and a courier; only "I have the food" and "I handed it over" are about a delivery. Both are deliberate taps rather than geofence inferences — being at a counter is not the same as having what is on it, and a delivery confirmed by proximity is one somebody can be paid for without doing.
- **The order rating is not passed to `dispatch.complete`.** A two-star for a cold burger is about the food as much as the trip, and must not land on the record of whoever carried it. Couriers therefore have no rating input yet — a deliberate gap, not an oversight.
- **`courierProgress` is measured, not simulated**: the courier's reported position projected onto the restaurant→door line, falling back to 0 when any of the three is unknown. The old timeline drew a pin that moved smoothly whatever the courier was doing, including standing still.
- **`OrderPlaced` gained `merchantOwnerId` and finally got a consumer.** Nobody had to be told about an order before, because a timeline started cooking on its own; a kitchen that is never told has an order it can only time out.
...
## 10. Trust & safety baseline — **BUILT 2026-07-31** (blocking, reporting, account deletion) **+ 2026-08-02** (SOS, share links, emergency contacts)

Built as specified for the three App Store blockers; the two ride-dependent items (SOS, live share links) followed in **Phase 3 M5**, once there was a trip for them to describe. Deviations worth recording:

**The safety pack — refinements the M5 build made (2026-08-02):**

- **Share links are their own platform module, `share`, and hold a pointer rather than a payload.** SPECS put `share_token(kind, refId, token, expiresAt)` inside the trust & safety work; what it needs to be is a module, because the payload has to be *rendered on every read* or a "live tracking link" shows where the car was when the link was made. So `share` stores the kind, the id and the secret, and asks the owning module for the view through a **`ShareableResource`** SPI — the fourth instance of the `ConversationGuard` / `ModeratableContent` / `WorkerEligibility` shape, and the same direction (`travel → share`). A kind nothing on the classpath renders is simply not shareable, and a token pointing at one 404s.
- **The token is the whole authentication, so two things are load-bearing.** 192 bits from `SecureRandom`, never derived from the ride id; and a payload deliberately thin enough to hand a stranger — first names only, a plate, a position, an ETA, a destination. No handle, no profile id, no phone number, no fare. **Every failure is the same 404** (expired, revoked, unknown kind, never existed), because a stream of guesses that can tell them apart is a stream of guesses that is learning something.
- **Minting is idempotent per (kind, ref, owner, reason), and only ever extends.** Tapping Share twice must not scatter two secrets for one trip — the second would be un-revokable by anybody already holding the first. Re-sharing a trip running late pushes the deadline out; nothing shortens it, because that would silently cut off the person already watching.
- **A link outlives its trip by a grace (`safety.share.grace.minutes`, 60).** A link that dies at the kerb dies at the one moment the person watching cares. The TTL is derived from the trip's own duration estimate rather than a flat clock: a link that outlives a short trip is harmless, one that dies during a long trip is the feature not working.
- **The server never dials.** SOS returns the local emergency number (**CONFIG** `safety.emergency.number`, default 112 — it works across the EU and most GSM networks, which is the right answer when we do not know where somebody is) and the *app* puts it under a thumb. A backend placing an emergency call on somebody's behalf is a claim about integration with emergency services this system has not earned, and the spec's own "no PSAP integration claimed" is the same instinct.
- **The flag is written first, and cannot be un-raised.** The ride row is flushed before a link is minted or an event published, so an alarm is never lost to a downstream outage. And there is no "cancel SOS": the flag is what an operator sees, and an alarm that can be talked away is one somebody under pressure will be talked into removing.
- **Emergency contacts are a phone number first and a GojoGo account second** (`profile.emergency_contact`, max **CONFIG** 5). The person you would call in an emergency is usually not a user of the app you happen to be sitting in, so a contacts list that only held accounts would be empty at the moment it mattered. A **linked** contact gets a push instantly; an unlinked one is reached by the phone's own message composer, pre-filled with wording the *server* wrote — which is also the honest channel while SNS SMS is still sandboxed (PROGRESS "Needs YOU" #3), and the one whose message arrives from a number the recipient recognises. Deleting an account takes the list with it.
- **The SOS operator surface is read-only.** `GET /v1/travel/admin/sos` on the same terms as partner review and the moderation queue. There is no "resolve" button because there is no honest thing for one to mean: a trip in trouble is dealt with by a person picking up a phone, and a status changing in a queue would only make it look handled.
- **Not built:** SOS on a *delivery* (the `share` module is generic and `delivery` can implement `ShareableResource` whenever it wants one), a share link for content or stories, and universal-link routing for `https://gojogo.app/s/{token}` — the endpoint is live and the path is what the route will map to, but the iOS association file is an account decision.

Deviations from the original three:
