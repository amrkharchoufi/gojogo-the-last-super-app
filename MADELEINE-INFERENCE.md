...
2. **Grant the Fargate task role `bedrock-mantle:CreateInference`.** **Corrected 2026-08-08 during M2, and this item previously said something false.** It claimed `bedrock:InvokeModel` on a foundation-model ARN covers the `bedrock-mantle` Chat Completions path because "both authorize as InvokeModel". They do not. A correctly signed request from M2's `ModelClient` is refused with:

   ```
   not authorized to perform: bedrock-mantle:CreateInference
   on resource: arn:aws:bedrock-mantle:us-east-1:578109959809:project/default
   ```

   Different service prefix, different action, and a **project** resource with no model in it — so the mantle surface authorizes per project and there is no model-scoping to be had on that statement. Both statements are now in [fargate-stack.ts](infra/lib/fargate-stack.ts) after the `sns:Publish` grant: `bedrock-mantle:CreateInference` on `project/*`, which is what actually runs, plus a scoped `bedrock:InvokeModel` kept precautionarily because IAM reports only the *first* denial and so proves nothing about whether mantle then authorizes the underlying invocation too. Ships with `./scripts/deploy-backend.sh --infra`.
   - The `InvokeModel` statement's ARNs are now `qwen.*` and `mistral.*` (both foundation-model and `us.` inference-profile forms), replacing the `meta.llama*` prefixes that predate §9's decision — Meta was never invocable and is not what ships. Prefixes rather than the two exact ids because mantle's model ids are not the foundation-model ids: `qwen.qwen3-235b-a22b-2507` does not appear in `list-foundation-models` at all.
   - **Bedrock needs no secret** — the task role *is* the credential. So there is no new `-c …SecretArn` flag, and therefore no new instance of the 2026-07-30 forgotten-flag failure. But note what this item's own history shows: "cannot ship half-configured" is not the same as "cannot ship broken". The failure mode here was never a silent one — every turn 401s and the assistant reports an honest `turn_error` — it was simply a prerequisite marked **Written** that nobody re-checked, because a ticked box is the last thing anyone tests. **Not verified end to end:** `gojogo-builder` cannot invoke the endpoint and cannot call `iam:SimulateCustomPolicy`, so the corrected policy is right by construction against the service's own error string, not by a green run. The first deployed turn is the real test.
