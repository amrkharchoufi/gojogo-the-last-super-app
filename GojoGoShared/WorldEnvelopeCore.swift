...
//
// This file is the half that has no idea an app exists: it is compiled into the
// notification extension too (Phase G), which opens exactly the same envelopes
// with exactly the same rules. The app's own entry points — the ones that know
// about stores, screens and who is signed in — live beside it in the app target.
...
enum WorldEnvelope {
    ...
    /// Opens a body of either version against an explicit local identity and
    /// store. `sender` is the *peer's* profile id — sessions are per-peer, not
    /// per-conversation.
    ///
    /// For v2 this consumes the message key: never call it twice on the same
    /// body, from either process. `WorldEnvelopeOpener` is what guarantees the
    /// callers don't.
    static func open(version: Int, body: String, from sender: UUID, as me: UUID,
        ...
        if version >= sealedVersion {
            plain = try WorldSignalSession.open(sealed,
                                                from: try WorldSignalSession.address(for: sender),
                                                as: try WorldSignalSession.address(for: me),
                                                store: store,
                                                identityChanged: identityChanged)
            WorldSignalSession.recordSealedMessage(from: sender)
        }
