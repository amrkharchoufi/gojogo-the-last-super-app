...
**Live packages today:** `com.gojogo.auth`, `profile`, `social`, `media`, `music`, `messaging`, `notifications`, `economy`, `delivery`, `watch`, `partner` (plus app-level `SecurityConfig` / `ApiExceptionHandler`). `notifications` is the busiest cross-module event consumer (`social`'s `UserFollowed` / `PostLiked` / `PostCommented` / `StoryReacted` / `StoryReplied`, `messaging`'s `MessageSent` for chat push, and `partner`'s `PartnerReviewed` for an approval or rejection). `economy` publishes `ListingCreated` and `delivery` publishes `OrderPlaced` / `OrderStatusChanged` — none consumed yet (search and order push are later slices).

**Public cross-module APIs in use:** `ProfileApi`, `MediaApi` + `MediaDocumentApi`, `MusicApi`, `MessagingApi` (+ `ConversationContext`), `SocialGraphApi`, and `MerchantProvisioningApi`. Each is deliberately narrow — a consumer gets the one verb it needs, never the module's model. Where a vertical needs to *change* something another module owns (following, a chat, a catalog entry), it either sends the user to that module's own surface or asks for exactly one operation and gets an id back.
...
| `MerchantPartnerView`, `MerchantDashboardView` | Vertical | `partner` (+ owner-scoped `delivery`) | `partner` | **Merchant onboarding — built 2b M6, not yet deployed, entry point hidden.** Apply → private KYC upload → human review → an approval provisions a `delivery.merchant` the owner then runs (storefront + menu editor) |
