...
- Platform/vertical schemas as modules land: `profile.*`, `social.*`, `media.*`, `messaging.*` (Postgres side where needed), `notifications.*`, `economy.*`, `delivery.*`, `travel.*`, `partner.*`, `dispatch.*` (or Redis-primary with a thin Postgres ledger), `services.*`, `payments.*` (the GoJo Wallet double-entry ledger; Stripe is source of truth for external charges), `kyc.*` (identity **verdicts** only — the documents live with the IDV vendor, deliberately not here).
