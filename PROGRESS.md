...
5. **Gojo Admin owns restaurant creation** (decided 2026-07-27) — the in-app apply flow is gone. Until Gojo Admin exists, the only way to create a restaurant is the token-guarded review surface by hand, and it has **no admin-side create endpoint yet** (the applicant endpoints are caller-scoped). **You are the KYC reviewer** either way: approvals are a human decision and there is no admin role in this app. After the M6 deploy, read the generated token once — `aws secretsmanager get-secret-value --secret-id gojogo/partner-admin-token --query SecretString --output text` — and keep it somewhere safe; nothing else can approve a restaurant, which means nothing else can put one in GojoDelivery. Runbook and the exact curls are in the Phase 2b M6 section. Worth deciding before launch: whether a real reviewer is you, or whether this becomes a small internal web tool.
6. **Cost/scaling decisions** — RDS is publicly reachable (a NAT Gateway ~$32/mo removes that); App Runner bills ~24/7 (`aws apprunner pause-service` when idle). Your call.
7. **Git** — nothing has been committed this session; commit when you're happy.
8. ~~**IAM for the new deploy path**~~ — **applied 2026-07-25.** `iam:PassRole` for `ecs-tasks.amazonaws.com` is granted, so `scripts/deploy-backend.sh` (and CI) can register a task definition again. Not yet re-exercised end to end: the first deploy after this will be the proof.
...
**Not here on purpose: incoming orders.** A merchant can't accept or advance an order, because `OrderFulfilmentJob` walks every order along a simulated timeline — a kitchen button the job immediately overrules is worse than no button. That surface arrives with Phase 3 `dispatch`, which replaces the job.
