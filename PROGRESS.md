...
## Phase 2d — Watch backend + post deletion (built 2026-07-25, NOT deployed)
...
**Still to do:** deploy (`V18` + the module), then a two-user curl E2E — publish, feed, distinct-viewer count across two accounts, subscriber count reflecting a follow, owner-only 403 on edit/delete, and the post-delete rollback. There is no local Postgres in this environment, so nothing here has run against a live database; `ModularityTests` (the boundary check) is green.
